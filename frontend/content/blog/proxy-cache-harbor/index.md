...
This is usually enough for small clusters, but dynamic and big clusters face issues. A simple solution for that is to implement a proxy cache project with [Harbor](https://goharbor.io/). Notice that there are several befenits, besides avoiding the low rate pull limit from Dockerhub, of implementing an image proxy cache, such as:
- **Reduce egress traffic**: NAT is usually charged by amount of traffic
...
- Checking if update was updated. If it wasn't it serves cache, if it was it will proxy the request to Dockerhub (notice that this commonly happens if you use `latest` tag)
- If DockerHub is not reachable, it will serve the cache image

## Local Kubernetes setup

I will demo this locally using [Kind](). I highly recommend you to check my blog post on [how to setup a local Kubernetes cluster with LoadBalancer and name resolution in MacOs and Linux](https://felipetrindade.com/kubernetes-ingress-load-balancer/). I will reuse all the components present in the blog post. Since I don't want to repeate myself and keep the things dry, I highly recommend you to read it (or skim the text).

## Demo time

### Basic local Kubernetes cluster setup

Once again, some of this steps were explained in detail on my previous blog post, I won't go deeper into the explanation here
...
and announce using L2 ARP protocol
...
You should see that and external-ip is being used by the NGINX service (`kubectl get service -n ingress-nginx`).

### Harbor installation

The only configuration that needs to be changed for this demo (at end of this blog post I will comment several improvement that must be done in a real production environment) is related to the ingress, but I will also modify the admin password to be `admin` by simplicity (the classic `admin`/`admin`).
...
Now we are able to create a new proxy cache project using the registry we just created

![Screenshot of Harbor new proxy cache project](./proxy-cache-project.png)


Instead of manually manage your Harbor configuration, a better approach is to create all of this using infrastructure as a code. I highly recommend Terraform/OpenTofu in these cases. You can simply use the Harbor provider created by the community and have all configuration as a code. Let's create a simple Terraform module for that
...
3) Deploy an application which image is already presented in the node
...
- Pointing the image to the desired registry and repository. This looks obvious, but what I want to say is: we are not going to directly use DockerHub images, we are going to use Harbor as registry. This means that we will need to add a [prefix](https://goharbor.io/docs/2.1.0/administration/configure-proxy-cache/) to DockerHub images to point to the Harbor project (`<harbor_server_name>/<proxy_cache_name>`), in our use case this will be equivalent to `harbor.ingress.local/proxy_cache`.
...
For simplicity I'll use the admin user credentials (that has permissions to pull images from the proxy cache project). Notice that this secret needs to be create in every namespace that our apps will require the usage of Harbor registry, in our case we will deploy all apps in the `default` namespace. Let's create this secret by running
...
Let's deploy an NGINX kubernetes Deployment manifest using Kustomize. A common pattern when using Kustomize is to have a `base` folder that will be reused by the `overlays` folder (usually the overlays folders represent an environment, e.g. `development`). We would apply generators and patches in the overlays folder but always using the base resources as reference.
...
When using Helm, there are two possibilies: you created the Helm chart, you are using someone else Helm chart. I'm not going to detail the first option, since you have total control over the templates used you can simply add the variable to add the `imagePullSecrets` and also a variable to add a prefix to the image. The second option might be scary: what if the chart owner didn't add this variable? Well, it's possibly but unlikely since these are very common things in a well built Helm chart. Let's use [Grafana chart](https://artifacthub.io/packages/helm/grafana/grafana) as example.

As you can see on the `default values` section, there are two variables that we are going to use: `global.imageRegistry`, `global.imagePullSecrets`. So let's create our custom values for this chart:
...
#### Deploy an application which image is already presented in the node
Now we are going to deploy an application using an image directly from Dockerhub and modifying to use Harbor proxy cache and see how it's going to behave, it might surprise you.
...
Let's apply that and see that indeed it worked as expected. The pod is running. Now, let's modify this deployment a bit to use Harbor proxy cache and the registry private credential.
...
Now, let's rollout the deployment (or simply delete the pod, a downtime is irrelevant here) and check if the image will now be present in Harbor.
...
## Cya
Hope you enjoyed this blog post! Please, don't copy and paste this to directly deploy to production environment. There are SEVERAL things that you would need to do in order be in a production-ready state. Here are a few tips before deploying this in a real environment.
...
2) **Configure external database for Harbor**: The configurations for Harbor (such as SSO, users) is all stored in the database. By default, the Helm chart deploys a Postgres database for you but it's more secure to use an external Postgres database (it can even be a database that you run inside Kubernetes with an Operator).
